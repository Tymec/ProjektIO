...
export const orderApi = api.injectEndpoints({
  endpoints: (build) => ({
    ...
    myOrders: build.query({
      ...
      providesTags: (result) =>
        result ? [...result.map(({ _id: id }) => ({ type: 'Order', id })), 'Order'] : ['Order']
    }),
    getOrder: build.query({
      query: (orderId) => `/orders/${orderId}/`,
      providesTags: (result) => (result ? [{ type: 'Order', id: result._id }, 'Order'] : ['Order'])
    }),
    deleteOrder: build.mutation({
      ...
      invalidatesTags: (result) =>
        result ? [{ type: 'Order', id: result._id }, 'Order'] : ['Order']
    }),
